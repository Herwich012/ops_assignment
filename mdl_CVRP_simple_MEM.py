...
MEM = fcs.aploc(data.MEMhub_s)

#%%Model parameters
rnd = np.random
rnd.seed(0)
n = len(MEM)-1                  #number of clients
...
c = {(i,j): round(distance.distance(MEM[i,1:3],MEM[j,1:3]).km,2) for i,j in A} #cost (distance)
